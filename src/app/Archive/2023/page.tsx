export default function Archive2023() {
  return (
    <main className="bg-white text-neutral-900">

      {/* Hero Section */}
      <section className="relative min-h-[70vh] flex items-center justify-center px-6 text-center overflow-hidden bg-neutral-900">

        <div className="absolute inset-0 bg-black/40"></div>

        <div className="relative z-10 max-w-4xl text-rose-600">
          <h1 className="text-4xl md:text-5xl tracking-tight leading-tight">
            প্রতিবিম্ব
            <br />
            <span className="text-white text-4xl">প্রথম সংখ্যা, ২০২৩</span>
          </h1>
        </div>

      </section>


      {/* About Edition */}
      <section className="py-24 px-6 bg-rose-50">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">

          <div>
            <h2 className="text-4xl md:text-5xl font-semibold tracking-tight">
              The 2023 Edition
            </h2>

          </div>

          <div className="space-y-6 text-lg text-neutral-700 leading-relaxed">
            <p>
              Identity is not given. It is formed, questioned, and rediscovered.
            </p>

            <p>
              This edition of Pratibimba explores the many layers of self — 
              personal, cultural, and collective.
            </p>
          </div>

        </div>
      </section>


      {/* Editorial Board */}
      <section className="py-24 px-6">

          <h2 className="text-4xl md:text-5xl font-semibold tracking-tight mb-16 text-center">
            Editorial Board
          </h2>

          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-16">

      {/* Member 1 */}
      <div className="text-center">
        <div className="relative w-40 h-40 mx-auto rounded-full overflow-hidden shadow-lg bg-rose-300">
          <img
            src="/avatar-default.svg"
            alt="President"
            className="w-50 h-50 object-contain"
          />
        </div>
        <h3 className="mt-6 text-xl font-semibold">Ms. Monami Chakraborty</h3>
      </div>

      {/* Member 2 */}
      <div className="text-center">
        <div className="relative w-40 h-40 mx-auto rounded-full overflow-hidden shadow-lg bg-rose-300">
          <img
            src="/avatar-default.svg"
            alt="President"
            className="w-50 h-50 object-contain"
          />
        </div>
        <h3 className="mt-6 text-xl font-semibold">Mr. Debarshi Chakrabarty</h3>
      </div>

      {/* Member 3 */}
      <div className="text-center">
        <div className="relative w-40 h-40 mx-auto rounded-full overflow-hidden shadow-lg bg-rose-300">
          <img
            src="/avatar-default.svg"
            alt="President"
            className="w-50 h-50 object-contain"
          />
        </div>
        <h3 className="mt-6 text-xl font-semibold">Mr. Arghadeep Nandi</h3>
      </div>

      {/* Member 1 */}
      <div className="text-center">
        <div className="relative w-40 h-40 mx-auto rounded-full overflow-hidden shadow-lg bg-rose-300">
          <img
            src="/avatar-default.svg"
            alt="President"
            className="w-50 h-50 object-contain"
          />
        </div>
        <h3 className="mt-6 text-xl font-semibold">Mr. Pijush Nandy</h3>
      </div>

      {/* Member 1 */}
      <div className="text-center">
        <div className="relative w-40 h-40 mx-auto rounded-full overflow-hidden shadow-lg bg-rose-300">
          <img
            src="/avatar-default.svg"
            alt="President"
            className="w-50 h-50 object-contain"
          />
        </div>
        <h3 className="mt-6 text-xl font-semibold">Ms. Rachana Dey</h3>
      </div>

      <div className="text-center">
        <div className="relative w-40 h-40 mx-auto rounded-full overflow-hidden shadow-lg bg-rose-300">
          <img
            src="/avatar-default.svg"
            alt="President"
            className="w-50 h-50 object-contain"
          />
        </div>
        <h3 className="mt-6 text-xl font-semibold">Mr. Ranjan Ray</h3>
      </div>

      <div className="text-center">
        <div className="relative w-40 h-40 mx-auto rounded-full overflow-hidden shadow-lg bg-rose-300">
          <img
            src="/avatar-default.svg"
            alt="President"
            className="w-50 h-50 object-contain"
          />
        </div>
        <h3 className="mt-6 text-xl font-semibold">Prof. Rituparno Sen</h3>
      </div>

      <div className="text-center">
        <div className="relative w-40 h-40 mx-auto rounded-full overflow-hidden shadow-lg bg-rose-300">
          <img
            src="/avatar-default.svg"
            alt="President"
            className="w-50 h-50 object-contain"
          />
        </div>
        <h3 className="mt-6 text-xl font-semibold">Dr. Nilesh Mazumder</h3>
      </div>

    <div className="text-center">
        <div className="relative w-40 h-40 mx-auto rounded-full overflow-hidden shadow-lg bg-rose-300">
          <img
            src="/avatar-default.svg"
            alt="President"
            className="w-50 h-50 object-contain"
          />
        </div>
        <h3 className="mt-6 text-xl font-semibold">Dr. Prasenjit Paul</h3>
      </div>

      <div className="text-center">
        <div className="relative w-40 h-40 mx-auto rounded-full overflow-hidden shadow-lg bg-rose-300">
          <img
            src="/avatar-default.svg"
            alt="President"
            className="w-50 h-50 object-contain"
          />
        </div>
        <h3 className="mt-6 text-xl font-semibold">Mr. Biswarup Das</h3>
      </div>
        </div>
      </section>


      {/* Highlights / Philosophy */}
      <section className="py-24 px-6 bg-rose-50">
        <div className="max-w-6xl mx-auto text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-semibold tracking-tight">
            Highlights
          </h2>
        </div>

        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-12">

          <div>
            <h3 className="text-2xl font-semibold text-rose-600">Self</h3>
            <p className="mt-4 text-neutral-700 leading-relaxed">
              Stories that dive into personal journeys and inner reflections.
            </p>
          </div>

          <div>
            <h3 className="text-2xl font-semibold text-rose-600">Culture</h3>
            <p className="mt-4 text-neutral-700 leading-relaxed">
              Exploring identity through language, heritage, and expression.
            </p>
          </div>

          <div>
            <h3 className="text-2xl font-semibold text-rose-600">Society</h3>
            <p className="mt-4 text-neutral-700 leading-relaxed">
              Questioning norms and redefining collective identity.
            </p>
          </div>

        </div>
      </section>


      {/* Flipbook Section */}
      <section className="py-24 px-6 text-center">
        <div className="max-w-4xl mx-auto">

          <h2 className="text-4xl md:text-5xl font-semibold tracking-tight mb-10">
            Read the Issue
          </h2>

          <p className="text-neutral-600 mb-10">
            Experience the complete 2023 edition.
          </p>

          {/* Replace with your Flipbook */}
          <div className="flex justify-center">
  <div className="w-full max-w-xl aspect-[3/4] rounded-lg overflow-hidden shadow-2xl">
    <iframe
      src="https://heyzine.com/flip-book/4075387fe0.html"
      className="w-full h-full"
      allowFullScreen
      allow="clipboard-write"
      style={{ border: 0 }}
    />
  </div>
</div>
        </div>
      </section>


      {/* Closing Section */}
      <section className="py-32 px-6 text-center">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-4xl md:text-6xl font-bold tracking-tight">
            To understand who we are,
            <br />
            <span className="text-rose-600">
              we must first question it.
            </span>
          </h2>
        </div>
      </section>

    </main>
  );
}
